import optuna
import copy
from hpo_base import hyperparamters_dict, optimisation_objective, df_user, df_item, user_item_affinities, prepare_data_mappers, rating_scale
...
def rmse_objective(trial):
    ...
    gaussian_noise = trial.suggest_uniform('gaussian_noise', 0.1, 0.35)
    ...
    rmse, _ = optimisation_objective(params, algo, report, objective)
    return rmse
...
def ndcg_objective(trial):
    ...
    report = get_reporter(trial)
    _, ndcg = optimisation_objective(params, algo, report, objective)
    return 1 - ndcg
