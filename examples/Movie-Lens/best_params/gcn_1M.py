params = {
  1: dict(n_dims=64, combining_factor=0.1,
                           collaborative_params=dict(
                               prediction_network_params=dict(lr=0.03, epochs=75, batch_size=1024,
                                                              network_depth=3,
                                                              gaussian_noise=0.25, conv_depth=2,
                                                              kernel_l2=1e-9, dropout=0.0,),
                               user_item_params=dict(lr=0.1, epochs=30, batch_size=256, l2=0.0001,
                                                     gcn_lr=0.00075, gcn_epochs=20, gcn_layers=2, gcn_dropout=0.0,
                                                     gcn_kernel_l2=1e-8, gcn_batch_size=1024, conv_depth=2,
                                                     margin=1.0,
                                                     gaussian_noise=0.15,
                                                     node2vec_params=dict(walk_length=40, num_walks=20, window=5, iter=2, p=0.5, q=0.5)))),
  2: dict(n_dims=64, combining_factor=0.1,
                           collaborative_params=dict(
                               prediction_network_params=dict(lr=0.03, epochs=75, batch_size=1024,
                                                              network_depth=3,
                                                              gaussian_noise=0.25, conv_depth=2,
                                                              kernel_l2=1e-9, dropout=0.0,),
                               user_item_params=dict(lr=0.1, epochs=30, batch_size=256, l2=0.0001,
                                                     gcn_lr=0.00075, gcn_epochs=20, gcn_layers=2, gcn_dropout=0.0,
                                                     gcn_kernel_l2=1e-8, gcn_batch_size=1024, conv_depth=2,
                                                     margin=1.0,
                                                     gaussian_noise=0.15,
                                                     node2vec_params=dict(walk_length=40, num_walks=20, window=5, iter=2, p=0.5, q=0.5)))),
  3: dict(n_dims=64, combining_factor=0.1,
                  collaborative_params=dict(
                    prediction_network_params=dict(lr=0.03, epochs=75, batch_size=1024,
                                                   network_depth=3,
                                                   gaussian_noise=0.25, conv_depth=2,
                                                   kernel_l2=1e-9, dropout=0.0,),
                    user_item_params=dict(lr=0.1, epochs=30, batch_size=256, l2=0.0001,
                                          gcn_lr=0.00075, gcn_epochs=20, gcn_layers=2, gcn_dropout=0.0,
                                          gcn_kernel_l2=1e-8, gcn_batch_size=1024, conv_depth=2,
                                          margin=1.0,
                                          gaussian_noise=0.15,
                                          node2vec_params=dict(walk_length=40, num_walks=20, window=5, iter=2, p=0.5, q=0.5)))),
  4: dict(n_dims=112, combining_factor=0.1,
                           collaborative_params=dict(
                               prediction_network_params=dict(lr=0.05, epochs=70, batch_size=1536,
                                                              network_depth=4,
                                                              gaussian_noise=0.275, conv_depth=4,
                                                              kernel_l2=1e-9, dropout=0.0,),
                               user_item_params=dict(lr=0.1, epochs=30, batch_size=256, l2=0.0001,
                                                     gcn_lr=0.00075, gcn_epochs=20, gcn_layers=2, gcn_dropout=0.0,
                                                     gcn_kernel_l2=1e-8, gcn_batch_size=1024,
                                                     margin=1.0,
                                                     gaussian_noise=0.15,
                                                     node2vec_params=dict(walk_length=40, num_walks=20, window=5, iter=2, p=0.5, q=0.5)))),
  5: dict(n_dims=112, combining_factor=0.1,
                           collaborative_params=dict(
                               prediction_network_params=dict(lr=0.2, epochs=15, batch_size=1536,
                                                              network_depth=4,
                                                              gaussian_noise=0.15, conv_depth=4,
                                                              kernel_l2=1e-9, dropout=0.0,),
                               user_item_params=dict(lr=0.025, epochs=5, batch_size=256, l2=0.0001,
                                                     gcn_lr=0.0005, gcn_epochs=3, gcn_layers=2, gcn_dropout=0.0,
                                                     gcn_kernel_l2=1e-8, gcn_batch_size=1024, conv_depth=2,
                                                     margin=1.0,
                                                     gaussian_noise=0.25,
                                                     node2vec_params=dict(walk_length=20, num_walks=10, window=5, iter=2, p=1.0, q=0.25)))),
          }
